...
if($_REQUEST['eliminacion']==1) {//confirmacion de insercion  
	$campos="estado_baremos='2'";
	$error=editar("baremos",$campos,'cod_baremos',$eli_codigo); 
	if ($error >=1)
...
if(!empty($busquedas)) { #codigo para buscar 
	...
	$where=" where estado_baremos = 1 and $busquedas";
}#codigo para buscar 
else{
	$where=" where estado_baremos = 1";
}

$sql="SELECT * FROM baremos 
INNER JOIN tipo_baremos ON tipo_baremos.cod_tipob = baremos.tipo_baremos
INNER JOIN genero ON genero.cod_genero = baremos.genero_baremos
$where";
...
<tr>
<td valign="top" >
<form id="forma_total" name="forma_total" method="post" action="man_baremos.php">
                  <table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" >
                    <tr>
                      <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td> 
						  <?php if ($insertar==1) {?>
					  	  <img src="imagenes/agregar.png"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_baremos.php?codigo=0&editar=<?php echo $editar?>&insertar=<?php echo $insertar?>&eliminar=<?php echo $eliminar?>'"/>
					  	  <?php } ?></td>
                          ...
                          <td valign="middle"><select name="campos" class="textfieldlista" id="campos" >
                            ...
                            <option value="nom_tipob">TIPO BAREMOS</option>
                            <option value="nom_genero">GENERO</option>
                            <option value="edad_baremos">EDAD</option>
                            <option value="valor_baremos">VALOR</option>
                            <option value="puntos_baremos">PUNTOS</option>
                    ...
                    <tr>
                      <td><table width="100%" border="0"  cellpadding="0">
						...
						echo "<tr style='display:none'><td ><form name='forma_0' action='man_baremos.php'>";
						echo "  </form> </td></tr>  ";						  
						$estilo="ctablablanc";
						$estilo="ctablagris";
						...
						while($db->next_row()) {
							if ($aux==0) { $estilo="ctablablanc"; $aux=1; $cambio_celda=$celda_blanca; }else { $estilo="ctablagris";  $cambio_celda=$celda_gris; $aux=0;}
							  echo "<tr class='$estilo' $cambio_celda> <form name='forma_$db->cod_baremos' action='man_baremos.php'>  ";
                echo "<td>$db->nom_tipob</td>";
						    echo "<td>$db->nom_genero</td> ";
                echo "<td>$db->edad_baremos</td> ";
                echo "<td>$db->valor_baremos</td>";
                echo "<td>$db->puntos_baremos</td> ";
                echo "<td aling='center' >"; 
							  echo "<table width='100%' border='0' cellspacing='0' cellpadding='0'>";
                echo 	" <tr>  <td align='center'> <input type='hidden' name='codigo' value='$db->cod_baremos'>";
							if ($editar==1)
							 	echo "<img src='imagenes/icoeditar.png' alt='Editar Registro' style='cursor:pointer'  onclick='document.forma_$db->cod_baremos.submit()'/></td>";
							else 
								echo "<img src='imagenes/e_icoeditar.png'   /></td>";
                            echo 	"<td align='center'>";
							if ($eliminar==1)
								echo"<img src='imagenes/icoeliminar.png'  alt='Eliminar Registro' height='16' style='cursor:pointer' onclick=confirmar($db->cod_baremos) /></td> ";
							else
...
<form name="forma" method="post" action="con_baremos.php">
