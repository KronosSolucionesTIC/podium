...
if($_REQUEST['eliminacion']==1) {//confirmacion de insercion  
	$campos="estado_enc ='2'";
	$error=editar("m_encuentro",$campos,'cod_enc',$eli_codigo); 
	if ($error >=1)
...
if(!empty($busquedas)) { #codigo para buscar 
	...
	$where=" where estado_enc = 1 and $busquedas";
}#codigo para buscar 
else{
	$where=" where estado_enc = 1";
}

$sql="SELECT * FROM m_encuentro 
INNER JOIN categoria ON categoria.cod_cat = m_encuentro.cat_enc
INNER JOIN campeonato ON campeonato.cod_cam = m_encuentro.cod_cam
$where
ORDER BY fec_enc DESC";
...
<tr>
<td valign="top" >
<form id="forma_total" name="forma_total" method="post" action="man_encuentros.php">
                  <table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" >
                    <tr>
                      <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td> 
						  <?php if ($insertar==1) {?>
					  	  <img src="imagenes/agregar.png"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_encuentros.php?codigo=0&editar=<?php echo $editar?>&insertar=<?php echo $insertar?>&eliminar=<?php echo $eliminar?>'"/>
					  	  <?php } ?></td>
                          ...
                          <td valign="middle"><select name="campos" class="textfieldlista" id="campos" >
                            ...
                            <option value="fec_enc">FECHA</option>
                    ...
                    <tr>
                      <td><table width="100%" border="0"  cellpadding="0">
						...
						echo "<tr style='display:none'><td ><form name='forma_0' action='man_encuentros.php'>";
						echo "  </form> </td></tr>  ";						  
						$estilo="ctablablanc";
						$estilo="ctablagris";
						...
						while($db->next_row()) {
							...
							echo "<tr class='$estilo' $cambio_celda> <form name='forma_$db->cod_enc' action='man_encuentros.php'>  ";
						  echo "<td>$db->fec_enc</td> ";
              echo "<td>$db->nom_cat</td>";
              echo "<td>$db->nom_cam</td> ";
              echo "<td aling='center' >"; 
							echo 	"<table width='100%' border='0' cellspacing='0' cellpadding='0'>";
              echo 	" <tr>  <td align='center'> <input type='hidden' name='codigo' value='$db->cod_enc'>";
							if ($editar==1)
							 	echo "<img src='imagenes/icoeditar.png' alt='Editar Registro' style='cursor:pointer'  onclick='document.forma_$db->cod_enc.submit()'/></td>";
							else 
								echo "<img src='imagenes/e_icoeditar.png'   /></td>";
                            echo 	"<td align='center'>";
							if ($eliminar==1)
								echo"<img src='imagenes/icoeliminar.png'  alt='Eliminar Registro' height='16' style='cursor:pointer' onclick=confirmar($db->cod_enc) /></td> ";
							else
...
<form name="forma" method="post" action="con_encuentros.php">
