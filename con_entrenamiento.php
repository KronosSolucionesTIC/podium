...
if($_REQUEST['eliminacion']==1) {//confirmacion de insercion  
	$campos="estado_ent ='2'";
	$error=editar("m_entrenamiento",$campos,'cod_ent',$eli_codigo); 
	if ($error >=1)
...
if(!empty($busquedas)) {
  if($_REQUEST['campo'] == 'nombre'){
    ...
    $where="having $busquedas and estado_ent = 1";
  } 
  else{
    $busquedas=reemplazar_1($busquedas);
    $where="where $busquedas and estado_ent = 1";
  }
}
else{
  $where="where estado_ent = 1";
}

$sql="SELECT *,CONCAT(nom1_pro,' ',nom2_pro,' ',apel1_pro,' ',apel2_pro) as nombre FROM m_entrenamiento 
INNER JOIN categoria ON categoria.cod_cat = m_entrenamiento.cat_ent
INNER JOIN profesor ON profesor.cod_pro = m_entrenamiento.cod_pro
$where 
ORDER BY fec_ent DESC";
...
<tr>
<td valign="top" >
<form id="forma_total" name="forma_total" method="post" action="man_entrenamiento.php">
                    ...
                    <tr>
                      <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td> 
						  <?php if ($insertar==1) {?>
					  	  <img src="imagenes/agregar.png"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_entrenamiento.php?codigo=0&editar=<?php echo $editar?>&insertar=<?php echo $insertar?>&eliminar=<?php echo $eliminar?>'"/>
					  	  <?php } ?></td>
                          ...
                          <td valign="middle"><select name="campos" class="textfieldlista" id="campos" >
                            ...
                            <option value="fec_ent">Fecha</option>
                            <option value="nom_cat">Categoria</option>
                            <option value="nombre">Profesor</option>
                    ...
                    <tr>
                      <td><table width="100%" border="0"  cellpadding="0">
                        <tr>
                          ...
                          <td  class="ctablasup">PROFESOR</td>
                          <td  class="ctablasup">OPCIONES</td>
                        </tr>
						<?php 
						
						echo "<tr style='display:none'><td ><form name='forma_0' action='man_entrenamiento.php'>";
						echo "  </form> </td></tr>  ";						  
						$estilo="ctablablanc";
						$estilo="ctablagris";
						...
						while($db->next_row()) {
							...
							echo "<tr class='$estilo' $cambio_celda> <form name='forma_$db->cod_ent' action='man_entrenamiento.php'>  ";
						  echo "<td>$db->fec_ent</td> ";
              echo "<td>$db->nom_cat</td>";
              echo "<td>$db->nombre</td> ";
              echo "<td aling='center' >"; 
							echo 	"<table width='100%' border='0' cellspacing='0' cellpadding='0'>";
              echo 	" <tr>  <td align='center'> <input type='hidden' name='codigo' value='$db->cod_ent'>";
							if ($editar==1)
							 	echo "<img src='imagenes/icoeditar.png' alt='Editar Registro' style='cursor:pointer'  onclick='document.forma_$db->cod_ent.submit()'/></td>";
							else 
								echo "<img src='imagenes/e_icoeditar.png'   /></td>";
                            echo 	"<td align='center'>";
							if ($eliminar==1)
								echo"<img src='imagenes/icoeliminar.png'  alt='Eliminar Registro' height='16' style='cursor:pointer' onclick=confirmar($db->cod_ent) /></td> ";
							else
...
<form name="forma" method="post" action="con_entrenamiento.php">
