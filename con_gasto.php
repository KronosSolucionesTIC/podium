...
if($_REQUEST['eliminacion']==1) {//confirmacion de insercion  
  $campos="estado_gasto='2'";
  $error=editar("gastos",$campos,'cod_gasto',$eli_codigo); 
  if ($error >=1)
...
if(!empty($busquedas)) { #codigo para buscar 
  ...
  $where=" where estado_gasto = 1 and $busquedas";
}#codigo para buscar 
else{
  $where=" where estado_gasto = 1";
}


$sql="SELECT *,CONCAT(nom1_prov,' ',nom2_prov,' ',apel1_prov,' ',apel2_prov) as nombre FROM gastos
...
INNER JOIN proveedor ON proveedor.cod_prov = gastos.prov_gasto
  $where ORDER BY fecha_gasto DESC";
...
<form id="forma_total" name="forma_total" method="post" action="man_gastos.php">
                      ...
                      <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td valign="middle"><select name="campos" class="textfieldlista" id="campos" >
                             ...
                             <option value="fecha_gasto">Fecha</option>
							               <option value="nom_gas">Tipo de Gasto</option>
							               <option value="valor_gasto">Valor</option>
              ...
              echo "<td >$db->nombre</td>";
               ...
               //impresion  
              echo "<td align='center'><img src='imagenes/mirar.png' width='16' height='16'  style=\"cursor:pointer\" onClick=\"imprimir_inf('recibo_egreso.php',$db->cod_gasto,'grande')\" /></td>"; 
