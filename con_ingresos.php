...
if($_REQUEST['eliminacion']==1) {//confirmacion de insercion  
  $campos="estado_ing='2'";
  $error=editar("ingresos",$campos,'cod_ing',$eli_codigo); 
  if ($error >=1)
...
if(!empty($_REQUEST['busquedas'])) { #codigo para buscar 
  $busquedas=reemplazar_1($busquedas);
  $where=" where estado_ing = 1 and $busquedas";
}#codigo para buscar 
else{
  $where=" where estado_ing = 1";
}
...
<form id="forma_total" name="forma_total" method="post" action="man_ingresos.php">
                    ...
                    <tr>
                      <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td width="19"> 
              <?php  if ($insertar==1) {?>
                <img src="imagenes/agregar.png" width="16" height="16"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_ingreso.php?codigo=0&editar=<?php echo $editar?>&insertar=<?php echo $insertar?>&eliminar=<?php echo $eliminar?>'"/>
                <?php  } ?></td>
              ...
              <option value="nom_gas">Tipo de ingreso</option>
            ...
            while($db->next_row()) {
              ...
              echo "<td><div align='right'>".number_format($db->val_ing,0,",",".")."</div></td>";
              echo "<td >$db->nombre</td>"; 
                            echo "<td aling='center' >";              
              echo  "<table width='100%' border='0' cellspacing='0' cellpadding='0'>";
                            echo  " <tr>  <td align='center'> <input type='hidden' name='codigo' value='$db->cod_ing'>";
              if ($editar==1)
                echo "<img src='imagenes/icoeditar.png' alt='Editar Registro' width='16' height='16' style='cursor:pointer'  onclick='document.forma_$db->cod_ing.submit()'/></td>";
              else 
                echo "<img src='imagenes/e_icoeditar.png' width='16' height='16'  /></td>";
                            echo  "<td align='center'>";
              if ($eliminar==1)
                echo"<img src='imagenes/icoeliminar.png' width='16' alt='Eliminar Registro' height='16' style='cursor:pointer' onclick=confirmar($db->cod_ing) /></td> ";
              else
                echo"<img src='imagenes/e_icoeliminar.png' width='16' height='16'  /></td> ";

              //impresion  
              echo "<td align='center'><img src='imagenes/mirar.png' width='16' height='16'  style=\"cursor:pointer\" onClick=\"imprimir_inf('recibo_ingreso.php',$db->cod_ing,'grande')\" /></td>"; 
