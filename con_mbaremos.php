...
if($_REQUEST['eliminacion']==1) {//confirmacion de insercion  
	$campos="estado_mbaremos='2'";
	$error=editar("m_baremos",$campos,'cod_mbaremos',$eli_codigo); 
	if ($error >=1)
...
if(!empty($busquedas)) { #codigo para buscar 
	...
	$where=" where estado_mbaremos = 1 and $busquedas";
}#codigo para buscar 
else{
	$where=" where estado_mbaremos = 1";
}
...
<tr>
<td valign="top" >
<form id="forma_total" name="forma_total" method="post" action="man_mbaremos.php">
                  <table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" >
                    <tr>
                      <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td> 
						  <?php if ($insertar==1) {?>
					  	  <img src="imagenes/agregar.png"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_mbaremos.php?codigo=0&editar=<?php echo $editar?>&insertar=<?php echo $insertar?>&eliminar=<?php echo $eliminar?>'"/>
					  	  <?php } ?></td>
                          ...
                          <td valign="middle"><select name="campos" class="textfieldlista" id="campos" >
                            ...
                            <option value="cod_mbaremos">CODIGO</option>
                            <option value="fec_mbaremos">FECHA</option>
                    ...
                    <tr>
                      <td><table width="100%" border="0"  cellpadding="0">
						...
						echo "<tr style='display:none'><td ><form name='forma_0' action='man_mbaremos.php'>";
						echo "  </form> </td></tr>  ";						  
						$estilo="ctablablanc";
						$estilo="ctablagris";
						...
						while($db->next_row()) {
							if ($aux==0) { $estilo="ctablablanc"; $aux=1; $cambio_celda=$celda_blanca; }else { $estilo="ctablagris";  $cambio_celda=$celda_gris; $aux=0;}
							  echo "<tr class='$estilo' $cambio_celda> <form name='forma_$db->cod_mbaremos' action='man_mbaremos.php'>  ";
                echo "<td>$db->cod_mbaremos</td>";
						    echo "<td>$db->fec_mbaremos</td> ";
                echo "<td aling='center' >"; 
							  echo "<table width='100%' border='0' cellspacing='0' cellpadding='0'>";
                echo 	" <tr>  <td align='center'> <input type='hidden' name='codigo' value='$db->cod_mbaremos'>";
							if ($editar==1)
							 	echo "<img src='imagenes/icoeditar.png' alt='Editar Registro' style='cursor:pointer'  onclick='document.forma_$db->cod_mbaremos.submit()'/></td>";
							else 
								echo "<img src='imagenes/e_icoeditar.png'   /></td>";
                            echo 	"<td align='center'>";
							if ($eliminar==1)
								echo"<img src='imagenes/icoeliminar.png'  alt='Eliminar Registro' height='16' style='cursor:pointer' onclick=confirmar($db->cod_mbaremos) /></td> ";
							else
...
<form name="forma" method="post" action="con_mbaremos.php">
