...
if($_REQUEST['eliminacion']==1) {//confirmacion de insercion  
	$campos="estado_nom='2'";
	$error=editar("m_nomina",$campos,'cod_nom',$eli_codigo); 
	if ($error >=1)
...
if(!empty($busquedas)) { #codigo para buscar 
	...
	$where=" where estado_nom = 1 and $busquedas";
}#codigo para buscar 
else{
	$where=" where estado_nom = 1";
}

$sql="SELECT * FROM m_nomina 
INNER JOIN campeonato ON campeonato.cod_cam = m_nomina.cam_nom
INNER JOIN categoria ON categoria.cod_cat = m_nomina.cat_nom
$where";
                    ...
                    <tr>
                      <td><table width="90%" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td> 
						  <?php if ($insertar==1) {?>
					  	  <img src="imagenes/agregar.png"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_nomina.php?codigo=0&editar=<?php echo $editar?>&insertar=<?php echo $insertar?>&eliminar=<?php echo $eliminar?>'"/>
					  	  <?php } ?></td>
                    ...
                    <tr>
                      <td><table width="90%" border="0"  cellpadding="0">
						...
						while($db->next_row()) {
							...
							echo "<tr class='$estilo' $cambio_celda> <form name='forma_$db->cod_nom' action='man_nomina.php'>  ";
              echo "<td>$db->nom_cam</td>";
						  echo "<td>$db->nom_cat</td> ";
                            echo "<td aling='center' >"; 
							echo 	"<table width='100%' border='0' cellspacing='0' cellpadding='0'>";
                            echo 	" <tr>  <td align='center'> <input type='hidden' name='codigo' value='$db->cod_nom'>";
							if ($editar==1)
							 	echo "<img src='imagenes/icoeditar.png' alt='Editar Registro' style='cursor:pointer'  onclick='document.forma_$db->cod_nom.submit()'/></td>";
							else 
								echo "<img src='imagenes/e_icoeditar.png'   /></td>";
                            echo 	"<td align='center'>";

							if ($eliminar==1)
								echo"<img src='imagenes/icoeliminar.png'  alt='Eliminar Registro' height='16' style='cursor:pointer' onclick=confirmar($db->cod_nom) /></td> ";
							else
								echo"<img src='imagenes/e_icoeliminar.png' /></td> ";

               //impresion  
              echo "<td align='center'><img src='imagenes/mirar.png' width='16' height='16'  style=\"cursor:pointer\" onClick=\"imprimir_inf('inf_arbitrajes.php',$db->cod_cam,'grande')\" /></td>"; 
