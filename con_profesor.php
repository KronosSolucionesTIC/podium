...
if($_REQUEST['eliminacion']==1) {//confirmacion de eliminacion 
	$campos="estado_pro ='2'";
	$error=editar("profesor",$campos,'cod_pro',$eli_codigo); 
	if ($error >=1)
...
if(!empty($_REQUEST['busquedas'])) {
  if($_REQUEST['campo'] == 'nombre'){
    ...
    $where="having $busquedas and estado_pro = 1";
  } 
  else{
    $busquedas=reemplazar_1($busquedas);
    $where="where $busquedas and estado_pro = 1";
  }
}
else{
...
$sql="SELECT *,CONCAT(nom1_pro,' ',nom2_pro,' ',apel1_pro,' ',apel2_pro) as nombre FROM profesor 
INNER JOIN tipo_identificacion ON tipo_identificacion.cod_tiden = profesor.tiden_pro
 $where";
#codigo para buscar 
...
<tr>
<td valign="top" >
<form id="forma_total" name="forma_total" method="post" action="man_profesor.php">
                    ...
                    <tr>
                      <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td width="17"> 
						  <?php if ($insertar==1) {?>
					  	  <img src="imagenes/agregar.png" width="16" height="16"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_profesor.php?codigo=0&editar=<?php echo $editar?>&insertar=<?php echo $insertar?>&eliminar=<?php echo $eliminar?>'"/>
					  	  <?php } ?></td>
               ...
               <td  class="ctablasup">OPCIONES</td>
						...
						echo "<tr style='display:none'><td ><form name='forma_0' action='man_profesor.php'>";
						echo "  </form> </td></tr>  ";						  
						$estilo="ctablablanc";
						$estilo="ctablagris";
						...
						while($db->next_row()) {
							...
							echo "<tr class='$estilo' $cambio_celda> <form name='forma_$db->cod_pro' action='man_profesor.php'>  ";
							echo "<td >$db->nombre</td>";
							echo "<td >$db->nom_tiden</td>";
							echo "<td >$db->id_pro</td>";
...
<form name="forma" method="post" action="con_profesor.php">
