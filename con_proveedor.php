...
if($_REQUEST['eliminacion']==1) {//confirmacion de eliminacion 
	$campos="estado_prov ='2'";
	$error=editar("proveedor",$campos,'cod_prov',$eli_codigo); 
	if ($error >=1)
...
if(!empty($_REQUEST['busquedas'])) {
  if($_REQUEST['campo'] == 'nombre'){
    ...
    $where="having $busquedas and estado_prov = 1";
  } 
  else{
    $busquedas=reemplazar_1($busquedas);
    $where="where $busquedas and estado_prov = 1";
  }
}
else{
  $where="where estado_prov = 1";
}

$sql="SELECT *,CONCAT(nom1_prov,' ',nom2_prov,' ',apel1_prov,' ',apel2_prov) as nombre FROM proveedor 
INNER JOIN tipo_identificacion ON tipo_identificacion.cod_tiden = proveedor.tiden_prov
 $where";
#codigo para buscar 
...
<form id="forma_total" name="forma_total" method="post" action="man_proveedor.php">
               ...
               <td  class="ctablasup">OPCIONES</td>
						...
						while($db->next_row()) {
							...
							echo "<tr class='$estilo' $cambio_celda> <form name='forma_$db->cod_prov' action='man_proveedor.php'>  ";
                            ...
                            echo 	" <tr>  <td align='center'> <input type='hidden' name='codigo' value='$db->cod_prov'>";
							if ($editar==1)
							 	echo "<img src='imagenes/icoeditar.png' alt='Editar Registro' width='16' height='16' style='cursor:pointer'  onclick='document.forma_$db->cod_prov.submit()'/></td>";
							else 
								echo "<img src='imagenes/e_icoeditar.png' width='16' height='16'  /></td>";
                            echo 	"<td align='center'>";
							if ($eliminar==1)
								echo"<img src='imagenes/icoeliminar.png' width='16' alt='Eliminar Registro' height='16' style='cursor:pointer' onclick=confirmar($db->cod_prov) /></td> ";
							else
