...
if($_REQUEST['eliminacion']==1) {//confirmacion de insercion  
	$campos="estado_ting='2'";
	$error=editar("tipo_ingresos",$campos,'cod_ting',$eli_codigo); 
	if ($error >=1)
...
if(!empty($busquedas)) { #codigo para buscar 
	...
	$where=" where estado_ting = 1 and $busquedas";
}#codigo para buscar 
else{
	$where=" where estado_ting = 1";
}

$sql="SELECT * FROM tipo_ingresos $where";
...
<tr>
<td valign="top" >
<form id="forma_total" name="forma_total" method="post" action="man_tipoingreso.php">
                  <table width="90%" border="0" cellspacing="0" cellpadding="0" align="center" >
                    <tr>
                      <td><table width="90%" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td> 
						  <? if ($insertar==1) {?>
					  	  <img src="imagenes/agregar.png"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_tipoingreso.php?codigo=0&editar=<?=$editar?>&insertar=<?=$insertar?>&eliminar=<?=$eliminar?>'"/>
					  	  <? } ?></td>
                    ...
                    <tr>
                      <td><table width="90%" border="0"  cellpadding="0">
                        <tr>
                          <td  class="ctablasup">TIPO INGRESO</td>
                          <td  class="ctablasup" >OPCIONES</td>
                        </tr>
						<? 
						
						echo "<tr style='display:none'><td ><form name='forma_0' action='man_tipoingreso.php'>";
						echo "  </form> </td></tr>  ";						  
						$estilo="ctablablanc";
						$estilo="ctablagris";
						...
						while($db->next_row()) {
							...
							echo "<tr class='$estilo' $cambio_celda> <form name='forma_$db->cod_ting' action='man_tipoingreso.php'>  ";
              echo "<td >$db->nom_ting</td>";
              echo "<td aling='center' >"; 
							echo 	"<table width='100%' border='0' cellspacing='0' cellpadding='0'>";
              echo 	" <tr>  <td align='center'> <input type='hidden' name='codigo' value='$db->cod_ting'>";
							if ($editar==1)
							  echo "<img src='imagenes/icoeditar.png' alt='Editar Registro' style='cursor:pointer'  onclick='document.forma_$db->cod_ting.submit()'/></td>";
							else 
								echo "<img src='imagenes/e_icoeditar.png'   /></td>";
                            echo 	"<td align='center'>";
							if ($eliminar==1)
								echo"<img src='imagenes/icoeliminar.png'  alt='Eliminar Registro' height='16' style='cursor:pointer' onclick=confirmar($db->cod_ting) /></td> ";
							else
...
<form name="forma" method="post" action="con_tipoingreso.php">
